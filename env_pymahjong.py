...
class MahjongEnv(gym.Env):
    ...
    ACTION_TYPES = (
        [pm.BaseAction.Discard] * MAHJONG_TILE_TYPES
        + [pm.BaseAction.Chi] * 3
        + [pm.BaseAction.Pon]
        + [pm.BaseAction.AnKan]
        + [pm.BaseAction.Kan]
    ...
    def __init__(self):
        ...
        self.oracle_observation_space = Box(
            dtype=bool,
            low=0,
            high=1,
            shape=[self.ORACLE_OBS_DIM, self.MAHJONG_TILE_TYPES],
        )
        self.full_observation_space = Box(
            dtype=bool,
            low=0,
            high=1,
    ...
    def _check_player(self, player_id):
        if not player_id == self.t.who_make_selection():
            raise ValueError(
                "You are trying to obtain information from a player who is not making decision !!!! \
    ...
    def step(self, player_id: int, action: int, specified_tile=None):
        ...
        if not player_id == self.get_curr_player_id():
            raise ValueError(
                "current acting player ID is {}, but you are trying to ask player {} to act !!".format(
        ...
        if not self.riichi_stage2:
            ...
            pm.encv1_encode_action(
                self.t, curr_pid, self.act_container
            )  # no need zeros

            if self.act_container[action] == 0:
                raise ValueError(
                    "Not an action in available actions! (use get_valid_actions(player_id))"
                )
            ...
            if not self.riichi_stage2:
                ...
                # However, this case should be very rare in normal play
                    ...
                    if specified_tile:
                        kan_tile_id = specified_tile
                    else:
                        kan_tile_id = np.random.choice(
                            np.argwhere(self.get_obs(curr_pid)[3]).flatten()
                        )
                    ...
                    if specified_tile:
                        ...
                        kan_tile_id = np.random.choice(
                            np.argwhere(
                                (np.sum(obs[:4], axis=0) == 1)
                                * (np.sum(obs[6:10], axis=0) == 3)
                            ).flatten()
                        )
                    ...
                    raise SystemError(
                        "This should not happen, please report to the authors"
                    )
            ...
            self.t.make_selection_from_action_basetile(
                action_type, [self.may_riichi_tile_id], False
            )
            self.riichi_stage2 = False
            self.may_riichi_tile_id = None
    ...
    def _get_obs_from_table(self, player_id):
        ...
        if self.riichi_stage2:
            pm.encv1_encode_table_riichi_step2(
                self.t, self.may_riichi_tile_id, self.obs_container
            )
    ...
    def get_valid_actions(self, nhot=False):
        if not self.riichi_stage2:
            self.act_container.fill(0)
            pm.encv1_encode_action(
                self.t, self.get_curr_player_id(), self.act_container
            )
            ...
            if (
                np.sum(
                    act_container[self.CHILEFT : self.PON + 1]
                    + act_container[self.MINKAN]
                    + act_container[self.RON]
                )
                > 0
            ):
                act_container[self.PASS_RESPONSE] = 1
            # elif self.t.players[0].riichi and act_container[self.TSUMO] > 0:
            #     act_container[self.PASS_RESPONSE] = 1
        else:
...
class SingleAgentMahjongEnv(gym.Env):
        ...
        if opponent_agent == "random":
            ...
            try:
                ...
                raise FileNotFoundError(
                    "opponent_agent should be 'random' or the path of a pre-trained model (You can download the pre-trained model from pymahjong github release: https://github.com/Agony5757/mahjong/releases "
                )
    ...
    def _proceed_until_agent_turn(self):
        ...
        ) and self.env.get_curr_player_id() != self.THIS_AGENT_ID:
            if self.opponent_agent == "random":
                aval_actions = self.env.get_valid_actions(nhot=False)
                self.env.step(
                    self.env.get_curr_player_id(), np.random.choice(aval_actions)
                )
            else:
                action_mask = self.env.get_valid_actions(nhot=True)
                obs = self.env.get_obs(self.env.get_curr_player_id())
                action = self.opponent_agent.select(
                    obs, action_mask=action_mask, greedy=True
                )
